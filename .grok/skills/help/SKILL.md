...
1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `X:\Grok_Build\.grok/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `X:\Grok_Build\.grok/docs/user-guide/`. The available guides are:
   ...
   `X:\Grok_Build\.grok/README.md` for the comprehensive reference.

3. To **modify config** for the user, edit `X:\Grok_Build\.grok/config.toml` with search_replace.

4. To **create a skill** for the user, create `X:\Grok_Build\.grok/skills/<name>/SKILL.md`
   (read `X:\Grok_Build\.grok/docs/user-guide/08-skills.md` for the SKILL.md format).